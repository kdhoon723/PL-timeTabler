...
- Primary navigation: 시간표 편집을 기본 화면으로 두고 작업 책임을 `과목`, `시간표`, `자동완성`, `신청 목록`, `졸업요건`으로 고정한다. `신청 목록`은 과목이 생긴 뒤 나타나는 완료 보조 행동이지 상시 주 내비게이션이 아니다.
- Core routes/screens:
...
- Content hierarchy:
  1. 현재 시간표 캔버스와 충돌 상태
  2. 과목 추가, 필수 추천, 내 전공·교양 탐색과 선택 목록
  3. 총 학점·공강일·빈 시간 요약
  4. 자동완성 조건과 후보 비교
  5. 과목이 생긴 뒤의 신청 목록·예비 과목·복사·내보내기
  6. 별도 화면의 졸업요건 충족·부족 상태와 공식 근거

### Responsibility boundaries

- **과목:** 검색, 필수 추천, 전공·교양 필터, 현재 시간표와 자동완성 후보 바구니를 소유한다. 후보 바구니는 독립 주 내비게이션이나 새 작업 단계가 아니라 과목 영역 안의 보조 공간이다.
- **시간표:** 격자, 충돌, 분반 교체, 선택 유지, 후보 미리보기만 소유한다. 학적 설정이나 데이터 해명 카드를 주 작업면에 상주시켜서는 안 된다.
- **자동완성:** 조건, 실행 상태, 후보 생성·비교만 소유한다. 수강신청 복사·내보내기·데이터 메타정보를 포함하지 않는다.
- **신청 목록:** 최종 선택 과목·예비 과목·주의 상태·과목코드 복사·PNG/PDF 내보내기만 소유한다. 실제 학교 수강신청을 수행하는 것처럼 표현하지 않는다.
- **졸업요건:** 장기 이수 현황과 공식 규칙을 소유하며 시간표 편집기와 별도 화면을 유지한다.
...
- **과목 도구는 캔버스를 보조한다:** 필수 추천과 전공·교양 탐색, 선택 과목은 과목 영역에서만 제공한다. 모바일에서는 dialog/sheet로, 1200px 이상에서는 왼쪽 aside로 제공한다.
- **현재 시간표와 후보를 섞지 않는다:** 격자에 배치된 과목은 `현재 시간표`, 아직 배치하지 않고 자동완성에만 사용할 과목은 `후보 바구니`에 표시한다. 제외 과목은 후보 바구니의 접힌 보조 목록으로 관리하고 정상 작업면을 차지하지 않는다.
- **후보는 과목 단위로 담는다:** 후보 바구니에 담을 때 특정 분반 선택을 강요하지 않는다. 검색 결과의 과목 단위 행동으로 담고, 실제 분반은 자동완성이 모든 공식 분반에서 선택한다. 교수·분반을 유지하려는 경우에만 과목 상세에서 범위를 좁힌다.
- **자동완성은 독립 보조 도구다:** 768px 이상에서는 오른쪽 aside, 모바일에서는 이름 있는 modal dialog로 제공하며 다른 작업 단계의 기능을 끼워 넣지 않는다.
- **점진적 공개:** 필수 추천은 과목 영역에서 요청할 때 열고, 검색 결과는 최대 20개 과목 행만 먼저 보여주며 한 과목의 분반만 펼친다.
- **명령 복구:** 모바일 상단 `더보기`는 실행 취소·다시 실행·공유·졸업요건을 제공하고, 과목이 생기면 `신청 목록` 진입도 제공한다. PNG/PDF는 신청 목록 안에만 둔다. 편집 입력 밖에서는 Ctrl/Cmd+Z와 Shift+Ctrl/Cmd+Z를 지원한다.
...
- **학사 기준 설정:** 입학연도·처음 입학한 경로·학생 구분은 전공필수·교양필수 판정 정확도를 높이는 선택 설정이다. 과목 영역의 필수 추천과 프로필 메뉴에서 다시 진입한다.
...
- 모바일 시간표 위에는 필수과목 확인 또는 학과 설정처럼 맥락이 있을 때만 조용한 안내를 둔다. 과목 검색은 하단의 `과목 찾기` 한 개를 기본 행동으로 유지하고, 내 전공·교양선택은 검색 sheet 안의 빠른 필터로만 제공한다.
- 상단 앱바는 52px 높이로 학기 선택, 총 학점, 더보기만 배치한다. 로고나 큰 제목으로 세로 공간을 소비하지 않는다.
- 시간표 격자는 화면 본문을 차지하며 시간 축 36px와 월~금 5개 열이 한 화면 폭에 들어와야 한다.
- 과목 블록을 탭하면 상세·분반 교체·선택 유지·삭제가 바텀시트로 열린다. 블록 안에 작은 조작 버튼을 여러 개 넣지 않는다. 모든 모바일 바텀시트는 상단 drag handle을 아래로 72px 이상 끌면 닫히며, 헤더 제스처가 브라우저 pull-to-refresh로 전달되지 않는다.
- `꼭 포함`은 과목만 고정하고 분반은 자동 조정한다. `교수 유지`는 같은 교수의 분반 안에서, `현재 수업`은 선택한 교수·시간·분반 그대로 자동완성한다.
- 고정 범위는 `자동 / 교수 유지 / 현재 수업`의 짧은 단일 선택으로 제공한다. 같은 교수가 담당하는 다른 분반이 있을 때만 `교수 유지`를 노출한다.
- `과목 추가`는 하단 내비게이션 위의 명확한 단일 기본 행동으로 유지한다. 자동 생성은 같은 중요도의 두 번째 고정 버튼으로 경쟁시키지 않는다.
- 후보 추가는 별도의 전역 기본 버튼으로 경쟁시키지 않는다. 데스크톱은 과목 영역의 `후보 바구니`, 모바일은 자동완성 dialog의 첫 영역에서 `후보 담기`로 진입한다.
- 검색 시트는 최대 92dvh이며 검색창·필터 요약은 상단에 고정하고 결과만 스크롤한다. `내 전공`과 `교양선택`은 검색창 아래의 빠른 필터, 이수구분·요일은 `세부 필터` disclosure로 제공한다. 아무 필터도 누르지 않은 상태를 전체 과목으로 취급하고, 활성 필터가 있을 때만 `필터 초기화`를 노출한다.
- 자동완성 도구는 닫혔을 때 접근성·focus 흐름에서 사라지는 이름 있는 modal dialog다. 열 때 닫기 버튼으로 focus를 옮기고 닫기·Escape·browser Back 모두 정확한 trigger로 focus를 돌린다.
- 선택 과목이 하나 이상이면 `신청 목록 보기`를 시간표 요약 행동으로 노출한다. 신청 목록은 별도 sheet이며 자동완성 dialog에 포함하지 않는다.
...
- 768~1199px: 시간표·과목 바로가기와 자동완성 패널을 2열로 배치한다. 선택 과목은 시간표 아래 요약으로 유지한다.
- 1200px 이상: 과목·선택 목록 280px, 시간표 `minmax(560px, 1fr)`, 자동완성 320px의 3영역을 기본으로 한다.
...
- 과목 색상은 DJPic에 검증된 에브리타임 계열 20색을 그대로 사용하고, 월요일 이른 수업부터 주간 마지막 수업까지 시간순으로 고유 과목에 반복 배정한다. 다크 테마에서도 같은 불투명 색면과 검정 전경을 유지하고 모든 조합의 텍스트 대비를 5:1 이상으로 유지한다. 과목 블록은 루트의 작성자 색상 체계(`light`/`dark`)를 상속해야 하며 다크 환경에서 `only light`로 고정하지 않는다. Chromium·Whale의 강제 다크 엔진은 `only light`를 단순 `light`로 처리할 수 있어 밝은 과목색을 다시 어둡게 변환할 수 있기 때문이다.
- 같은 과목의 분반은 같은 hue를 사용하고 선택 유지·충돌은 별도 테두리와 아이콘으로 구분한다.
...
- Existing components to reuse: `TimetableGrid`, `SelectedCourseList`, `PreferencesPanel`, `OptimizerPanel`, `RegistrationChecklist`
- New/changed components:
  ...
  - `SelectedCourseList`: 선택 과목, 자동 조정 범위, 분반 교체, 삭제
  ...
  - `CourseEntryPanel`: 한 개의 `과목 찾기` 행동과 조건부 필수과목·학과 설정 안내를 분리하는 과목 진입점
  - `CandidateCourseBasket`: 자동완성에만 사용할 후보와 제외 과목을 현재 시간표와 분리해 관리하고 후보 검색으로 진입하는 공간
  - `RequiredCourseSheet` + `RequiredCoursePanel`: 전공필수·교양필수와 실제 개설 분반, 배치 상태만 보여주는 독립 확인 도구. 시간표 작성 순서나 전공선택·교양선택 탐색 단계를 내부에 섞지 않는다.
  - `ApplicationListSheet`: 최종 과목코드·예비 과목·복사·PNG/PDF를 한곳에 묶는 완료 보조 화면
  - `RequirementNavigator`: 입학연도·학과별 졸업요건과 부족 영역
  - `SourceEvidence`: 규칙의 원문, 적용 대상, 기준일 표시
- Variants and states: 기본, 선택, 교수 유지, 현재 수업 유지, 충돌, 시간 미정, 강의실 미상, 미리보기 유지·추가·제외·교체 전·교체 후, drag source, official drop slot
...
- Terminology: `과목`, `분반`, `공강`, `빈 시간`, `꼭 포함`, `교수 유지`, `현재 수업`, `충돌`, `시간 미정`
- Microcopy rules:
  ...
  - 데이터가 없거나 학적 기준이 맞지 않을 때는 원인 한 문장과 수정·계속하기 행동만 먼저 보이고, 판정 예외는 접힌 안내로 둔다.
  - 단계처럼 보이는 항목은 실제 이동 버튼이어야 한다. 현재 단계만 비활성 표식으로 두고 이후 단계는 해당 필터가 적용된 과목 검색을 연다.
