...
- 과목 색상은 DJPic에 검증된 에브리타임 계열 20색을 그대로 사용하고, 월요일 이른 수업부터 주간 마지막 수업까지 시간순으로 고유 과목에 반복 배정한다. 다크 테마에서도 같은 불투명 색면과 검정 전경을 유지하고 브라우저 자동 다크 변환에서 과목 블록을 제외하며 모든 조합의 텍스트 대비를 5:1 이상으로 유지한다.
