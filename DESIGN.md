...
- 과목 색상은 에브리타임 계열의 중채도 10색을 시간표에 처음 들어온 과목 순서대로 반복 배정한다. 라이트·다크는 각 배경에서 선명하게 분리되는 색면과 고대비 전경을 별도로 정의하며 모든 조합의 텍스트 대비를 5:1 이상으로 유지한다.
...
- New/changed components:
  ...
  - `RequiredCourseSheet` + `RequiredCoursePanel`: 필수과목 확인 뒤 전공선택·교양선택 검색으로 즉시 이어지는 3단계 내비게이션
