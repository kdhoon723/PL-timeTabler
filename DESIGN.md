...
- 시간표 위에는 필수 추천·내 전공·교양을 여는 짧은 과목 바로가기만 둔다. 긴 필수과목 설명과 학적 오류는 요청한 sheet 안에서 처리한다.
- 상단 앱바는 52px 높이로 학기 선택, 총 학점, 더보기만 배치한다. 로고나 큰 제목으로 세로 공간을 소비하지 않는다.
- 시간표 격자는 화면 본문을 차지하며 시간 축 36px와 월~금 5개 열이 한 화면 폭에 들어와야 한다.
- 과목 블록을 탭하면 상세·분반 교체·선택 유지·삭제가 바텀시트로 열린다. 블록 안에 작은 조작 버튼을 여러 개 넣지 않는다.
...
- New/changed components:
  ...
  - `CourseEntryPanel`: 전체 과목·필수 추천·내 전공·교양의 단일 과목 진입점
