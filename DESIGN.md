...
- Core routes/screens:
  - 첫 방문 온보딩 overlay: 학과 → 이번 학기 기준 학년, 언제든 건너뛰기. 입학연도·입학 경로·학생 구분은 편집기에서 여는 선택형 `학사 기준 설정`으로 분리한다.
  - `/`: 시간표 편집기
...
- **시간표 기본 설정:** 첫 방문에는 학과·전공과 이번 학기 기준 학년만 받는다. 다른 학년 과목도 계속 탐색할 수 있음을 설명하고 바로 편집기로 이동한다.
...
- 기본 온보딩은 입학연도를 학년으로 역산하거나 입학 경로를 `신입학`으로 추정해 저장하지 않는다.
