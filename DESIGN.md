...
- Confirmed product decisions: 대진대 전용, 수동 편집 우선, 보조 추천 포함
...
- Primary navigation: MVP는 별도 다단계 내비게이션 없이 단일 편집 화면을 사용한다.
- Core routes/screens:
  - `/`: 시간표 편집기
  - 공유 링크는 후속 단계에서 쿼리 또는 짧은 식별자로 표현한다.
- Content hierarchy:
  1. 현재 시간표와 충돌 상태
  2. 과목 추가 버튼과 검색
  3. 총 학점·공강일·빈 시간 요약
...
- Tradeoffs: 화면 밀도보다 터치 정확성과 가독성을 우선하고, 완전 자동화보다 되돌릴 수 있는 보조 추천을 우선한다.
...
- [ ] 시간표 공유를 URL과 이미지 중 무엇부터 제공할까 / 제품 / MVP 후반 범위에 영향
