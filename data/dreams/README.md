...
`data/courses/courses-2026-1.json`은 별도 시점에 만든 제품 고정 fixture(1,576개)이고, 이 아카이브의 2026-1은 2026-07-14 포털 조회 결과(1,489개)다. 원본 시점과 포함 범위가 다르므로 새 아카이브가 기존 fixture를 자동으로 덮어쓰지 않는다.

배포 시 `timetabler-ingest`가 기존 fixture를 검증한 뒤 이 아카이브를 별도 역사 테이블에 적재한다. 이때 원본 gzip 바이트, 루트 메타데이터, 각 분반·학과 교육과정·대체/동일과목 JSON 객체를 모두 보존한다. 과목명·학수번호·학기·교수·학과 등의 검색 컬럼은 원본을 대체하지 않는 투영값이며, 사용자 이수내역은 원본 분반 ID와 당시 JSON 스냅샷을 함께 참조한다.
