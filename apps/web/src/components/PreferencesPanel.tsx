...
export function PreferencesPanel({ preferences, onChange }: Props) {
  ...
  return <section className="preferences-panel" aria-labelledby="preferences-title">
    ...
    <div className="preference-grid">
      <label><span>목표 학점</span><input type="number" min="1" max="30" value={preferences.targetCredits} onChange={(event) => onChange({ ...preferences, targetCredits: Number(event.target.value) })} /></label>
      <label><span>최소 학점</span><input type="number" min="1" max="30" value={preferences.minCredits} onChange={(event) => onChange({ ...preferences, minCredits: Number(event.target.value) })} /></label>
      <label><span>최대 학점</span><input type="number" min="1" max="30" value={preferences.maxCredits} onChange={(event) => onChange({ ...preferences, maxCredits: Number(event.target.value) })} /></label>
