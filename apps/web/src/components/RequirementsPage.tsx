...
import { loadCommonRules, loadCompletedCourses, loadDepartmentSources } from '../api/client'
import { academicUnitOverrideStatus, applicableRules, completedCourseMatches, type StudentType } from '../domain/requirements'
import type { AcademicProfile, CommonRules, DepartmentSource, MajorRequiredCourses, Section } from '../types'
...
export function RequirementsPage({ catalog, profile, majorRequired = null, authenticated = false, onBack, onAddCourse }: Props) {
  ...
  useEffect(() => {
    ...
    loadCompletedCourses().then(({ completedCourses, creditSummary }) => {
      setCompletedCredits(creditSummary.totalCredits)
      setLiberalCredits(creditSummary.liberalCredits)
      setMajorCredits(creditSummary.majorCredits)
      setAreaCredits(creditSummary.areaCredits)
      setCompletedNames(completedCourses.filter((item) => item.status === 'COMPLETED').map((item) => item.courseName).join('\n'))
    }).catch(() => { /* manual inputs remain available */ })
  }, [authenticated])
  ...
  return <main className="requirements-page">
    ...
    </div>{studentType === 'OTHER' && <p className="source-note">국내학생 전용 교양·필수과목 규칙은 제외했습니다. 국제협력대학 또는 소속 학과의 별도 기준을 확인하세요.</p>}{authenticated && <p className="source-note">마이페이지에 저장한 이수 완료 과목을 자동으로 반영했습니다.</p>}<label className="completed-field"><span>이수한 필수과목</span><textarea rows={3} value={completedNames} onChange={(event) => setCompletedNames(event.target.value)} placeholder="과목명을 쉼표나 줄바꿈으로 입력"/><small>최종 졸업 판정은 학교 포털과 소속 학과에서 반드시 확인하세요.</small></label></section>
