...
describe('academic profile consistency safeguard', () => {
  it('explains an unusual freshman grade and requires explicit acknowledgement', async () => {
    ...
    await userEvent.click(screen.getByRole('checkbox', { name: /현재 3학년이 맞습니다/ }))
    await userEvent.click(screen.getByRole('button', { name: '시간표 만들기' }))

    expect(onComplete).toHaveBeenCalledWith(expect.objectContaining({ currentGrade: 3, gradeMismatchAcknowledged: true }))
  })
