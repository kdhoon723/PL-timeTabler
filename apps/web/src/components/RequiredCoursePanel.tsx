...
import { supportsSectionGroup } from '../domain/profile'
...
export function RequiredCoursePanel({ profile, rules, majorRequired, catalog, items, sectionById, onEditProfile, onAddRequired }: Props) {
  ...
  const options = useMemo<RequiredOption[]>(() => {
    ...
    const major = (majorProgram?.courses ?? [])
      ...
      .map((course) => ({ key: `major-${course.courseCode}`, name: course.name, code: course.courseCode, source: 'MAJOR' as const, handbookPage: course.handbookPage, sections: byCode.get(course.courseCode) ?? [] }))
    ...
    const liberal = (liberalRule?.courses ?? []).map((course) => {
      ...
      return { key: `liberal-${course.name}`, name: course.name, code: sections[0]?.courseCode ?? null, source: 'LIBERAL' as const, sections }
    })
    return [...major, ...liberal]
  }, [catalog, majorRequired, majorRequirementsApplicable, profile, rules])
