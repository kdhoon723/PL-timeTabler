...
export function RequiredCourseSheet({ open, profile, rules, majorRequired, catalog, items, sectionById, onClose, onEditProfile, onAddRequired }: Props) {
  ...
  return <dialog ref={dialogRef} className="sheet required-course-sheet" aria-labelledby="required-course-sheet-title" onCancel={(event) => { event.preventDefault(); onClose() }}>
    <div className="sheet-header" {...sheetDrag}><div><h2 id="required-course-sheet-title">필수과목 확인</h2><p>전공필수와 교양필수의 개설 분반을 확인하세요.</p></div><button type="button" className="icon-button" onClick={onClose} aria-label="필수과목 확인 닫기"><CloseIcon /></button></div>
    <RequiredCoursePanel profile={profile} rules={rules} majorRequired={majorRequired} catalog={catalog} items={items} sectionById={sectionById} onEditProfile={onEditProfile} onAddRequired={onAddRequired} />
  </dialog>
}
