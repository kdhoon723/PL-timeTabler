...
| `GET` | `/api/v1/requirements/rules` | `200` | 프로필에 적용할 공통·학과·입학연도·전공경로별 졸업요건 규칙 | 파일 + PostgreSQL |
| `POST` | `/api/v1/requirements/evaluate` | `200` | 총학점·교양·전공 영역·필수과목과 수동 확인 항목 점검 | 파일 + PostgreSQL |
| `GET` | `/api/v1/requirements/recommendations` | `200` | 미이수 전공·교양필수 과목의 현재 학기 후보 | 파일 + PostgreSQL |
