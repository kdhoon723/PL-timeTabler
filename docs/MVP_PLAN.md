...
- MVP는 `packages/timetable-core`의 TypeScript 백트래킹·가지치기로 구현하고 Python·OR-Tools는 포함하지 않는다.
- 전체 자동 생성이나 복잡한 학사 제약으로 모바일 p95 500ms 기준을 지키지 못할 때만 별도 Python optimizer 컨테이너를 추가한다.
...
4. 충돌이 생긴 부분 조합은 즉시 제거하고 현재 상위 후보보다 나빠질 것이 확정된 분기도 중단한다.
5. 후보별 등교일, 빈 시간, 이른 수업, 변경 분반 수, 이동 부담을 계산한다.
6. 상위 3개 후보의 변경점과 개선 효과를 자연어 템플릿으로 설명한다.
...
- 최악 조건 fixture를 반복 실행해 p95 500ms를 넘으면 OR-Tools 도입 검토 이슈를 생성하되 MVP 구현에 즉시 추가하지 않는다.
