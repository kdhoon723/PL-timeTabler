...
test.describe('responsive timetable editor', () => {
  ...
  test('searches, adds, persists and edits a section without horizontal overflow', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /신청 목록 보기.*1개.*2학점/ })).toBeVisible()
    await page.getByRole('button', { name: /신청 목록 보기.*1개.*2학점/ }).click()
    await expect(page.getByRole('dialog', { name: '신청 목록' })).toContainText('922601-01')
    await expect(page.getByRole('dialog', { name: '신청 목록' })).toContainText('PNG 저장')
  ...
  test('lets a regular third-year student start without transfer or admission-route decisions', async ({ page }) => {
    await page.goto('/')
    ...
    await expect(page.getByText('신입학')).toHaveCount(0)
    await expect(page.getByRole('combobox', { name: /입학연도/ })).toHaveCount(0)
    const transfer = page.getByRole('checkbox', { name: /혹시 편입생인가요/ })
    await expect(transfer).toBeVisible()
    await expect(page.getByText('편입 기준으로 안내할게요')).toHaveCount(0)
    await transfer.click()
    await expect(page.getByText('편입 기준으로 안내할게요')).toBeVisible()
    await expect(page.getByRole('combobox', { name: /대진대 편입학년도/ })).toBeVisible()
    await transfer.click()
    await expect(page.getByText('편입 기준으로 안내할게요')).toHaveCount(0)
    const startButton = page.getByRole('button', { name: '3학년 시간표 만들기' })
  ...
  test('uses a real mobile tools dialog with close, Escape, Back, and exact trigger focus return', async ({ page }, testInfo) => {
    ...
    await openEditor(page)
    const trigger = page.getByRole('button', { name: /자동완성/ })
    await expect(page.getByRole('dialog', { name: '자동완성' })).toHaveCount(0)

    await trigger.click()
    const dialog = page.getByRole('dialog', { name: '자동완성' })
    await expect(dialog).toBeVisible()
    await expect(dialog.getByText('수강신청 준비')).toHaveCount(0)
    await expect(dialog.getByText('내보내기')).toHaveCount(0)
  ...
  test('keeps the tools as a static reachable aside on tablet and desktop', async ({ page }, testInfo) => {
    ...
    await openEditor(page)
    await expect(page.getByRole('complementary', { name: '자동완성' })).toBeVisible()
    await expect(page.getByRole('dialog', { name: '자동완성' })).toHaveCount(0)
  })
  ...
  test('exposes mobile recovery commands through the compact overflow', async ({ page }, testInfo) => {
    ...
    for (const name of ['실행 취소', '다시 실행', '시간표 공유', '졸업요건']) {
